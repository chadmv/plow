...
! function($) {
    ...
    var Tag = function(element, options) {
        ...
        this.values = $.grep($.map(this.element.val()
            .split(','), $.trim), function(value) {
    ...
    Tag.prototype = {
        ...
        show: function() {
            ...
            that.element.parent()
                .prepend(that.element.detach()
                .attr('type', 'hidden'))
            that.element.wrap($('<div class="tags">'))
                .parent()
                .on('click', function() {
                that.input.focus()
            })
            ...
            that.input = $('<input type="text">')
                ...
                that.element.siblings('.tag')
                    .removeClass('tag-important')
            })
                .on('keydown', function(event) {
                if (event.keyCode == 188 || event.keyCode == 13 || event.keyCode == 9) {
                    if ($.trim($(this)
                        .val()) && (!that.element.siblings('.typeahead')
                        .length || that.element.siblings('.typeahead')
                        .is(':hidden'))) {
                        if (event.keyCode != 9) event.preventDefault()
                        that.process()
                    } else if (event.keyCode == 188) {
                        if (!that.element.siblings('.typeahead')
                            .length || that.element.siblings('.typeahead')
                            .is(':hidden')) {
                            event.preventDefault()
                        } else {
                            that.input.data('typeahead')
                                .select()
                    ...
                    .val()) && event.keyCode == 8) {
                    var count = that.element.siblings('.tag')
                        .length
                    if (count) {
                ...
                matcher: function(value) {
                    return~value.toLowerCase()
                        .indexOf(this.query.toLowerCase()) && (that.inValues(value) == -1 || that.options.allowDuplicates)
                },
        ...
        createBadge: function(value) {
            ...
            $('<span class="tag">')
                ...
                .on('click', function() {
                that.remove(that.element.siblings('.tag')
                    .index($(this)
                    .closest('.tag')))
            }))
        ...
        remove: function(index) {
            if (index >= 0) {
                ...
                this.element.siblings('.tag:eq(' + index + ')')
                    .remove()
                this.element.val(this.values.join(', '))
            }
        },
        process: function() {
            var values = $.grep($.map(this.input.val()
                .split(','), $.trim), function(value) {
                return value.length > 0
            }),
                that = this
